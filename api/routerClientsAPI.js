...
routerClientsAPI.route('/clients')
	...
	.get((req, res) => {
		...
		console.log(new ClientObj(req.query).toSearch())
	...
	.put(function(req, res) {
		Client.findById(req.params.client_id, (err, client) => {
			if(err) {
				...
				let clientObj = new ClientObj(req.body).toSearch(); // toSearch() need if NOT COMPLETE obj properties in req.body, example only: { "email": "aav@bbs.ru" }
				Object.keys(clientObj).forEach(prop => { client[prop] = clientObj[prop]; });
